...
- Python 3.12
- Используется sqlite3(база)
- .env используется только для секретного ключа. Есть генератор нового в settings.py

1. Добавить товары в базу данных через скрипт:
    ```commandline
    python manage.py parse_widberries наименование товара
    ```
2. Запуск сервера
    ```commandline
...
3. Получить аналитику по добавленным товарам:
    - Заглавная страница
4. CRUD для товаров по API
   - GET /api/products
5. Данные подлежат фильтрации
...
По цене, рейтингу, количеству отзывов.<br>
